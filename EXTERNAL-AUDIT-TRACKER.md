...
- [x] DATA-15 — Public road-trip plans can expose private locations — decided: accepted risk, no change. "I don't care, this is a family app." (Checkbox was left unflipped in an earlier round despite the decision already being made — corrected here for consistency with how every other accepted-risk item in this tracker is marked.)
...
- [x] UX-19 — Calendar/map lack non-visual equivalents — reviewed with the user and declined: a real, genuinely bigger feature (a full non-visual/data-table equivalent for the calendar and road-trip map), not a bounded fix, and this is a small family-only tool rather than a public product with a compliance obligation. Not re-fixed unless raised again.
...
- [x] UX-21 — Installed PWA has no offline/update experience — reviewed with the user and declined entirely: building a real offline mode (a service worker, cached data, conflict handling for a money-tracking app) is a materially bigger feature with real correctness risk (stale balances shown as current), not a bounded fix, for a benefit the user didn't feel was worth it on a family tool. Not re-fixed unless raised again.
...
- [x] PERF-02 — Pages over-fetch complete datasets — reviewed with the user and declined: at this app's real scale (a handful of family users, a few hundred bank rows each) the extra columns per row are not a felt problem — no slow page, no timeout, just more bytes than strictly needed on an already-fast query. Investigated what a real fix on the 3 busiest pages (dashboard/Banks/Accounts) would actually take before declining: `banks/page.tsx` and `accounts/page.tsx` hand their fetched rows straight into the edit drawer/modal (`BankForm`'s `initial={editingBank}`, same pattern in `AccountModal`) without a second fetch on open — trimming the list query's columns would silently blank out drawer fields the moment it's opened, since the drawer shows nearly every column anyway. Fixing it right needs the drawer to fetch its own full row on open instead of reusing the list's data — a real, separate architecture change, not a column-list edit, for a performance win the user can't feel at this scale. Not re-fixed unless raised again.
...
- [x] INT-11 — Notification-default migration can't tell opt-out from untouched — reviewed with the user and declined: the one real incident this describes already happened in the past (a migration silently re-enabled emails for someone who'd deliberately turned them all off) and can't be undone retroactively. The only forward-looking fix would be a new "touched" marker column so a *future* default-changing migration doesn't repeat the mistake — declined as guarding against a scenario that may never recur, not worth a schema change for. Not re-fixed unless raised again.
...
- [x] GAP-02 — Exact addresses sent to public Nominatim against its own policy — reviewed and accepted, no change: the road-trip planner and address-change page already only send an address the user is actively typing (type-ahead autocomplete), never a bulk/background job hitting Nominatim on its own — the same low-volume, user-initiated pattern most small apps use this free service for. No code change.
- [x] GAP-03 — Road-trip candidate/budget/map models disagree — fixed the real, verifiable half. Confirmed by reading the code: the "Add more banks nearby" candidate list and the "search to add" list both ranked/estimated a candidate's cost via `cheapestInsertion` against a *flat, single continuous route* — but the actual day-budget bar and itinerary come from `buildMultiDayItinerary`, which resets the drive clock at each day boundary. For a multi-day trip these two models can genuinely disagree: a candidate that flatly looks like "+8 min" can in reality force an entire extra day (confirmed with a standalone repro: a stop that doesn't fit in the remaining daily window showed a trivial flat added-cost with zero indication it actually pushes the trip from 1 day to 2). Fixed by adding `estimateAddedCost()` (`RoadTripClient.tsx`) — it inserts the candidate into the route at its cheapest position (kept `cheapestInsertion` for choosing *where*, since that part was never the issue) and then re-runs the same `buildMultiDayItinerary()` the itinerary/budget bar already use, diffing the real drive-time and day-count against the current trip. Both candidate lists and the map's tooltip (which reads off `rankedCandidates`) now report the same honest number the budget bar would show if you actually added that bank — closing all three legs of "candidate/budget/map disagree." Deliberately did **not** re-run the joint branch optimizer (`chooseBranchesForRoute`) per candidate — that's the one piece still judged too expensive/risky to simulate for every candidate in the list (confirmed live: after actually adding a candidate, the real total was off by a few minutes from the estimate, traced to the branch optimizer re-picking locations for the whole set once the candidate joins it — a separate, much smaller, pre-existing effect the old flat model had identically, not something this fix introduced or made worse). Verified via a standalone script reproducing the "1 day → 2 days, invisible to the flat model" case, `tsc`/`npm run build`/`npm test` (84/84) all clean, and a live DEMO_MODE CDP pass: added 3 geographically distant must-visit banks, confirmed the itinerary correctly needed 2 days while budgeted for 1 (matching the new candidate estimates), added the top-ranked candidate, and confirmed the budget bar's real change matched the candidate's shown estimate to within the known branch-reselection margin — no console errors, no mobile overflow (375px).
...
- **Only 1 finding remains open** — TYPE-01 (generated DB types via the Supabase CLI), blocked on
  ...
  rather than a bounded fix; INT-11 was declined on the same reasoning — the one real incident already
  happened and can't be undone, and a forward-looking "touched" marker guards against a recurrence
  that may never happen; GAP-02 was reviewed and accepted as already-fine (only ever a user-initiated
  type-ahead request, never a background job); GAP-03 was the one item in this batch that got a real
...
findings were left, then asked to go through the remaining batch. Presented each of the 8 remaining
items grounded in the real code/tradeoffs (not the tracker's one-line description) rather than a blind
"fix or skip" list — GAP-02 (only ever a user-initiated type-ahead request, never a bulk job, so no
real gap against Nominatim's usage policy), UX-19/UX-21 (accessibility for the calendar/map, and
offline/update support for the installed PWA — both real, bigger features with no bounded fix), PERF-02
(explained in plain terms what "over-fetching" actually costs at this app's real scale — nothing felt,
since it's a handful of family users on a fast query, not a slow page), and INT-11 (the migration
incident already happened and can't be undone — the only remaining question is a forward-looking
"touched" marker for next time). User accepted GAP-02 as-is, declined UX-19/UX-21/INT-11 outright, and
after the user pushed back on PERF-02 specifically ("is it really gonna make it better? it seems fine
to me now") — investigated what a real fix on the 3 busiest pages would actually require before
declining: `banks/page.tsx`/`accounts/page.tsx` hand their fetched rows straight into the edit drawer
(`BankForm`'s `initial={editingBank}`) without a second fetch on open, so trimming the list query would
silently blank drawer fields — a real architecture change (fetch-on-open), not a column-list edit, for
a win the user can't feel at this scale. Declined on that basis.

For the last item, GAP-03 (road-trip planner's candidate list, day-budget bar, and map disagreeing with
each other), the user said "if it needs fixing then fix it" — delegating the judgment call. Read
`RoadTripClient.tsx`/`roadtrip.ts` to confirm the claim was real, not just a one-line guess: the
candidate-ranking lists estimated a bank's added cost via `cheapestInsertion` against a *flat, single
continuous route*, while the actual day-budget bar and itinerary came from `buildMultiDayItinerary`,
which resets the drive clock at every day boundary. Verified with a standalone repro that these two
models can genuinely disagree on a multi-day trip: a stop that flatly looked like "+8 min" actually
forced the trip from 1 day to 2, with zero indication from the flat estimate. Fixed by adding a shared
`estimateAddedCost()` in `RoadTripClient.tsx` — it inserts the candidate into the route at its cheapest
position (kept `cheapestInsertion` for choosing *where*, since that part was never wrong) and then
re-runs the same `buildMultiDayItinerary()` the budget bar already trusts, diffing the real drive-time
and day-count against the current trip — used by both the "Add more banks nearby" list and the
"search to add" list, and since the map's tooltip reads straight off the same ranked list, all three
legs of "candidate/budget/map disagree" are now fed by one consistent number. Deliberately did **not**
re-run the joint branch optimizer (`chooseBranchesForRoute`) per candidate for the same reason earlier
rounds left this alone — real cost/regression risk on a planner that's already been through 4+ rounds
of careful tuning; confirmed live that this leaves a small (few-minutes) residual gap between the
candidate's shown estimate and the real total once actually added, traced to the branch optimizer
re-picking locations for the whole set once the candidate joins it — a separate, smaller effect the old
flat model had identically, not something this fix introduced or made worse.

**Verification**: `tsc --noEmit`, `npm run build`, and `npm test` (84/84) all clean. GAP-03 is genuinely
UI-observable (the "+N min" numbers and red/not-red styling shown to the user), so it got a live
DEMO_MODE CDP pass rather than just a code read: added 3 geographically distant must-visit banks
(forcing a real multi-day split), confirmed the itinerary panel correctly flagged "this plan needs 2
days, but you set 1" while the budget bar showed the 1-day-configured total, read the top-ranked
candidate's shown estimate, clicked "Add," and confirmed the budget bar's real before/after change
matched the candidate's shown estimate to within the known, pre-existing branch-reselection margin
described above — zero console errors (one pre-existing, unrelated hydration-mismatch warning on this
page, from `AddressAutocomplete`'s id generation, not touched by or introduced by this change), no
mobile overflow (375px). `DEMO_MODE` was flipped to `true` for this verification and flipped back to
`false` before finishing, per the standing rule; the standalone Node repro script and headless Chromium
process were both cleaned up after. No changelog/Guide entry — this is a bug fix to existing behavior,
not a new user-visible feature, matching the standing features-only policy. **99 of 100 findings are
now closed** — TYPE-01 (generated DB types) is the only one left, blocked on this sandbox's own network
policy rather than any decision; see the item above.
