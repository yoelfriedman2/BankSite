...
import { X, Loader2, UploadCloud, FileSpreadsheet } from "lucide-react";
...
/** Parse a workbook buffer into ImportBank rows, tolerating preamble + header naming. */
function parseWorkbook(buf: ArrayBuffer): ImportBank[] {
  ...
  if (idx.name < 0) {
    throw new Error(
      'Could not find a "Name" column. Make sure the sheet has a header row with a bank name column.',
    );
  }
    ...
    >
      ...
      >
        ...
        <p className="mb-4 text-sm text-slate-500">
          Upload an Excel/CSV file. It should have a header row with a{" "}
          <span className="font-medium text-slate-700">Name</span> column;{" "}
          <span className="font-medium text-slate-700">
            Cert, City, State, Assets, Holding Company
          </span>{" "}
          are matched automatically. Existing banks (matched by FDIC cert) are
          updated; new ones are added as untracked.
        </p>
