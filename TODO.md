...
- **Run migration `0051_transaction_ledger.sql`.** Adds `account_balance_history.type` (nullable,
  additive) plus a new UPDATE RLS policy on that table (it was insert/delete-only before), and two new
  functions: `record_account_transaction` (the new "+ Add transaction" deposit/withdrawal entry point)
  and `edit_last_account_transaction` (fixes the single most-recent user-entered transaction, only
  when its type is deposit/withdrawal/correction — never a system-generated row). Also `create or
  ...
  migration 0047 is preserved automatically (no `SET` clause in the new bodies).
  **Degrades gracefully except for the two brand-new actions**: every existing balance-changing path
  (manual balance edit, monthly fee, interest, sweeps) keeps working exactly as it does today whether
  or not this has run — `getBalanceHistory` reads via `select("*")` and falls back to guessing a
  row's type from its `reason` text if the column doesn't exist yet. The two new actions
  (`recordAccountTransaction`/`editLastAccountTransaction`, wired to "+ Add transaction" and the
  latest-row edit button in `BalanceHistoryBox`) are the one exception — they call the new RPCs
  directly and return a friendly error ("function not found") until this runs, same as this project's
  other money-critical migrations (e.g. sweep transactions, 0034).
...
- ~~Run migration `0047_function_search_path_hardening.sql`~~ — **confirmed run 2026-08-05.** Closed
  ...
  `credit_monthly_interest`(`_with_history`), `sweep_accounts`, `return_sweep`,
  `refresh_bank_branches`, `update_account_balance`, `claim_check_number`, `append_activity_log`) by
  pinning `SET search_path = ''` on each — pure hardening, no behavior change (all 12 are `SECURITY
  INVOKER` and every table reference inside them is already schema-qualified), so nothing further to
  verify or test now that it's applied. The rest of that scan's findings need no action:
  `handle_new_user`'s "callable by anon" flag is a known false positive (it's a trigger function —
  Postgres rejects calling it outside a trigger context); `is_approved`'s is intentional (documented
  in its own migration 0036 comment — safe for anon, returns `false`); `rls_auto_enable` isn't a
  function this codebase created (not in any migration — looks Supabase-platform-internal, left
  alone); `pg_trgm` living in the `public` schema is cosmetic and moving it risks the migration-0045
  trigram indexes for no real benefit; and "leaked password protection disabled" is moot since login
  is Google/Microsoft OAuth only — no password auth provider is enabled, so there's no password to
  check.
...
- ~~APK packaging~~ — **done.** The site is wrapped as an installable Android TWA. Real signed
  ...
  and always outputs an unsigned, uninstallable package; "New signing key" only appears under
  "All Settings" on the Google Play tab, and using it doesn't obligate publishing to the Play
  Store). `public/.well-known/assetlinks.json` now has the real `sha256_cert_fingerprints` from
  that signed build. `signing.keystore` / `signing-key-info.txt` are intentionally **not** in this
